...
// defineProps<{ durations: [{ tip: string, tag: string }] }>()
  ...
  <table class="card heat-calendar-table">
    ...
    <tr style="height: 13px">
      ...
      <th v-for="(_,i) in 12" :key="i"
          :colspan="i == 11 ? weekCount - monthWeeks[i] : monthWeeks[i + 1] - monthWeeks[i]">
        {{ monthList[i] }}
      </th>
    </tr>
    </thead>
    ...
    <tr v-for="(_,dow) in 7" :key="dow">
      <td style="width: 32px">{{ dow % 2 == 1 ? dayOfWeekList[dow] : "" }}</td>
